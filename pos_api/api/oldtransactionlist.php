...
$sql2 = "SELECT p.ID , p.post_date , CONCAT( MAX( CASE WHEN pm.meta_key = '_billing_email' AND p.ID = pm.post_id THEN pm.meta_value END ) ,' ', MAX( CASE WHEN pm.meta_key = '_billing_first_name' AND p.ID = pm.post_id THEN pm.meta_value END ) ,' ', MAX( CASE WHEN pm.meta_key = '_billing_last_name' AND p.ID = pm.post_id THEN pm.meta_value END ) ,' ', MAX( CASE WHEN pm.meta_key = '_billing_phone' AND p.ID = pm.post_id THEN pm.meta_value END ) ) as 'customer', MAX( CASE WHEN pm.meta_key = '_order_total' AND p.ID = pm.post_id THEN pm.meta_value END ) AS 'total', MAX( CASE WHEN pm.meta_key = '_paid_date' AND p.ID = pm.post_id THEN pm.meta_value END ) AS 'paid_date' FROM wp_posts AS p JOIN wp_postmeta AS pm ON p.ID = pm.post_id JOIN wp_woocommerce_order_items AS oi ON p.ID = oi.order_id WHERE post_type = 'shop_order' $where GROUP BY p.ID Order By p.post_date DESC $where1"
$sql = "SELECT wp_posts.ID, wp_posts.post_date, wp_postmeta.meta_key, wp_postmeta.meta_value from wp_posts Left Join `wp_postmeta` ON (`wp_posts`.ID=`wp_postmeta`.post_id AND wp_postmeta.meta_key = '_customer_user') WHERE post_type = 'shop_order' $where Order By wp_posts.post_date DESC $where1";
...
while ($row = $result->fetch_assoc()) 
{
    ...
    $order[] = array(
        ...
        'id'=>$row['ID'],
        'customer'=>$row['customer'],// $row1c ? ($row1c['user_nicename'].' '.$row1c['fname'].' '.$row1c['lname']):'',
        'total' =>$row['total'] // number_format(($row1? $row1['meta_value'] : 0),2)//+($rowtx?$rowtx['amount']:0)
    );
    $i++;
}
