...
if(isset($_REQUEST['id'])){
	...
	$sql_grandTotal = "SELECT * from `wp_postmeta`
	WHERE post_id = '".$_REQUEST['id']."' AND meta_key='_order_total'";
	$result_grandTotal = $conn->query($sql_grandTotal);
	$result_grandTotal = $result_grandTotal->fetch_assoc();
	...
	$grandTotal = ($subtotal - $discount) + $shipping + $fee+$total_tax;


	if($grandTotal < $result_grandTotal['meta_value'] && $fee == 0){
		$fee = $result_grandTotal['meta_value'] - $grandTotal;
	}
	$grandTotal = $result_grandTotal['meta_value'];
